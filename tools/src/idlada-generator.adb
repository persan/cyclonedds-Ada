with CycloneDDS.Idl.Tree;
package body Idlada.Generator is
   use CycloneDDS.Idl.Tree;
   function Istok ( Str : String ; Len : Natural ; Toks : GNAT.Strings.String_List) return Integer is
   begin
      for I in Toks'Range loop
         if Str = Toks (I).all then
            return I;
         end if;
      end loop;
      return -1;
   ...
   --  static size_t istok(onst char *str, size_t len, const char **toks)
   --  {
   ...
   function Get_Cpp11_Base_Type (Node : Node_Type) return String is
   ...
   --    (void)user_data;
   begin
      case Idl_Type (Node) is
      when IDL_CHAR => return "char";
      when IDL_WCHAR => return "wchar";
      when IDL_BOOL => return "bool";
      when IDL_INT8 => return "int8_t";
      when IDL_UINT8| IDL_OCTET => return "uint8_t";
      when IDL_SHORT| IDL_INT16 => return "int16_t";
      when IDL_USHORT| IDL_UINT16 => return "uint16_t";
      when IDL_LONG| IDL_INT32 => return "int32_t";
      when IDL_ULONG| IDL_UINT32 => return "uint32_t";
      when IDL_LLONG| IDL_INT64 => return "int64_t";
      when IDL_ULLONG| IDL_UINT64 => return "uint64_t";
      when IDL_FLOAT => return "float";
      when IDL_DOUBLE => return "double";
      when IDL_LDOUBLE => return "long double";
      end case;
   end Get_Cpp11_Base_Type;
